<?php
	...
	$uid=$_GET['uid'];
	$sql="select * from tbl_friendlist where requesterid='$uid'";
	$rs=mysql_query($sql);
?>
...
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1" />
 <title></title>
    ...
    <body>
					...
					<div class="tj_wrapper">
						...
						<?php
							...
							while($row=mysql_fetch_array($rs)){
								$sql1="select * from tbl_user where id='".$row['acceptorid']."'";
								$rs1=mysql_query($sql1);
								$row1=mysql_fetch_array($rs1);
						?>
							<li>
								<a class="ballistic" href="oprofile.php?id=<?php echo $row1['id']; ?>" onclick="parent.closer('<?php echo $row1['id']; ?>');">
		...
		<?php if($temp==0){ ?>
			<p style="font-family:Verdana; font-size:18px">Not connected with anyone....</p>
		<?php } ?>
    </body>
</html>